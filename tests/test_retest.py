...
class TestRetestStore:
    ...
    def test_delete_participant_removes_all_their_passages(self, tmp_path):
        store = RetestStore(tmp_path / "retest.db")
        code, _ = store.save_passage(**_passage_kwargs())
        store.save_passage(**_passage_kwargs(participant_code=code))  # second passage, same code
        other_code, _ = store.save_passage(**_passage_kwargs(session_user_id="user-other"))

        deleted = store.delete_participant(code)

        assert deleted == 2
        assert store.passages_for(code) == []
        assert len(store.passages_for(other_code)) == 1  # untouched

    def test_delete_participant_unknown_code_deletes_nothing(self, tmp_path):
        store = RetestStore(tmp_path / "retest.db")
        store.save_passage(**_passage_kwargs())
        assert store.delete_participant("never-issued") == 0
...
class TestRetestEndpoints:
    ...
    def test_withdraw_deletes_saved_passages(self, monkeypatch, tmp_path):
        import api.main as main_module
        ...
        answers = _full_answers()
        r1 = client.post("/study/retest/save", json={"user_id": "u1", "answers": answers})
        code = r1.json()["participant_code"]

        r2 = client.post("/study/retest/withdraw", json={"participant_code": code})
        assert r2.status_code == 200
        assert r2.json()["deleted_passages"] == 1

        # A second withdraw call on the same (now-empty) code deletes nothing further.
        r3 = client.post("/study/retest/withdraw", json={"participant_code": code})
        assert r3.json()["deleted_passages"] == 0
