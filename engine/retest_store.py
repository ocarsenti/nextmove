...
class RetestStore:
    ...
    def delete_participant(self, participant_code: str) -> int:
        """Right to withdraw: deletes every passage for this code, immediately
        and entirely — not a soft flag. Returns the number of rows deleted."""
        with self._connect() as conn:
            cur = conn.execute(
                "DELETE FROM retest_passages WHERE participant_code = ?",
                (participant_code,),
            )
            conn.commit()
            return cur.rowcount
